...
module VCAP::CloudController
  module Encryptor
    ...
    class << self
      ...
      def decrypt(encrypted_input, salt, label=nil, encryption_iterations:)
        return unless encrypted_input

        key = key_to_use(label)

        decrypt_raw(encrypted_input, key, salt, encryption_iterations: encryption_iterations)
      end

      def decrypt_raw(encrypted_input, key, salt, encryption_iterations:)
        run_cipher(make_cipher.decrypt, Base64.decode64(encrypted_input), salt, key, encryption_iterations: encryption_iterations)
      end
      ...
      def run_cipher(cipher, input, salt, key, encryption_iterations:)
        if deprecated_short_salt?(salt)
          cipher.pkcs5_keyivgen(key, salt)
        else
          cipher.key = OpenSSL::PKCS5.pbkdf2_hmac(key, salt, encryption_iterations, 16, OpenSSL::Digest::SHA256.new)
          cipher.iv = salt
        end
      ...
      module ClassMethods
        ...
        def set_field_as_encrypted(field_name, options={})
          ...
          define_method "#{field_name}_with_encryption" do
            Encryptor.decrypt(send("#{field_name}_without_encryption"), send(salt_name), encryption_key_label, encryption_iterations: encryption_iterations)
          end
