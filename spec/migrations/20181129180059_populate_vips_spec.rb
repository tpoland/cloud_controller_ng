...
RSpec.describe 'populate VIPS for preexisting routes', isolation: :truncation do
  ...
  context "when some internal routes exist before the migration" do
    ...
    let!(:internal_route_1) { VCAP::CloudController::Route.make(host: 'meow', domain: internal_domain, vip_offset: 123) }   # these are set to silly values so we know the migration works
